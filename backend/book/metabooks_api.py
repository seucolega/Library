...
class MetabooksAPI:
    METABOOKS_ACCESS_KEYS = config("METABOOKS_ACCESS_KEYS", cast=Csv(), default=None)
    ...
    def request(
        ...
        headers = {"Authorization": f"{self.AUTHENTICATION_SCHEME} {self.tokens[call_type]}"}
        headers.update(kwargs.get("header", {}))

        r = self.session.request(method=method.value, url=url, headers=headers, **kwargs)
    ...
    def get(
        ...
        return self.request(Method.GET, url, uri, call_type, return_json, **kwargs)
    ...
    def post(
        ...
        return self.request(Method.POST, url, uri, call_type, return_json, **kwargs)
...
def get_product_json_from_ean(
        metabooks_instance: MetabooksAPI,
        product_ean: str
) -> dict:
